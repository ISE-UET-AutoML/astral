...
async def create_instance(instance: GcpInstance):
    # Lệnh Terraform để tạo các instance
    print(terraform_utils.create_terraform_file(instance, "gcp"))
    target = f"{instance.type}.{instance.resource_name}"

    try:
        terraform_utils.terraform_apply(target, "gcp")
        value = instance_manager.get_instance_info_by_resouce_name(
            instance.resource_name, "gcp"
        )
...
async def get_instance_info(instance_id: str):
    ...
    info = instance_manager.get_instance_info_by_id(instance_id, "gcp")
    return {
...
async def delete_instance(instance_id: str):
    # Xóa instance
    info = instance_manager.get_instance_info_by_id(instance_id, "gcp")
    target = f"google_compute_instance.{info["name"]}"
    try:
        terraform_utils.terraform_destroy(target, "gcp")
